...
const ToastClose = <T extends ValidComponent = "button">(
  ...
  return (
    <ToastPrimitive.CloseButton
      class={cn(
        "text-foreground/50 group-[.destructive]:text-destructive-foreground group-[.error]:text-error-foreground group-[.success]:text-success-foreground group-[.warning]:text-warning-foreground absolute top-2 right-2 rounded-md p-1 opacity-0 transition-opacity group-hover:opacity-100 focus:opacity-100 focus:ring-2 focus:outline-none",
        local.class
      )}
      {...others}
    >
...
const ToastTitle = <T extends ValidComponent = "div">(
  ...
  return (
    <ToastPrimitive.Title class={cn("text-sm font-semibold", local.class)} {...others} />
  );
};
