...
const NavigationMenuTrigger = <T extends ValidComponent = "button">(
  ...
  return (
    <NavigationMenuPrimitive.Trigger
      class={cn(
        "group/trigger bg-background hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground data-[active]:bg-accent/50 data-[expanded]:bg-accent/50 inline-flex h-10 w-full items-center justify-center rounded-md px-4 py-2 text-sm font-medium whitespace-nowrap transition-colors focus:outline-none disabled:pointer-events-none disabled:opacity-50",
        local.class
      )}
      {...others}
    />
...
const NavigationMenuViewport = <T extends ValidComponent = "li">(
  ...
  return (
    <NavigationMenuPrimitive.Viewport
      class={cn(
        // base settings
        "border-border bg-popover pointer-events-none z-[1000] flex h-[var(--kb-navigation-menu-viewport-height)] w-[var(--kb-navigation-menu-viewport-width)] origin-[var(--kb-menu-content-transform-origin)] items-center justify-center overflow-x-clip overflow-y-visible rounded-md border opacity-0 shadow-lg data-[expanded]:pointer-events-auto data-[expanded]:rounded-md data-[orientation=vertical]:overflow-x-visible data-[orientation=vertical]:overflow-y-clip",
        // animate
        "animate-content-hide data-[expanded]:animate-content-show transition-[width,height] duration-200 ease-in data-[expanded]:opacity-100 data-[expanded]:ease-out",
        local.class
      )}
      {...others}
    />
...
const NavigationMenuLink = <T extends ValidComponent = "a">(
  ...
  return (
    <NavigationMenuPrimitive.Item
      class={cn(
        "hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground block space-y-1 rounded-md p-3 leading-none no-underline transition-colors outline-none select-none",
        local.class
      )}
      {...others}
    />
